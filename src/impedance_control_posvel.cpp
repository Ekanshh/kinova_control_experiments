...
void impedance_control_posvel(k_api::Base::BaseClient* base,
    ...
    std::cout << "Gravity vector: " << GRAVITY(0) << ", " << GRAVITY(1) << ", " << GRAVITY(2)
              << std::endl;

    const KDL::JntArray ZERO_ARRAY(ACTUATOR_COUNT);
    const KDL::Wrenches ZERO_WRENCHES(pArmChain.getNrOfSegments(), KDL::Wrench::Zero());
    KDL::JntArray jntCmdTorques(ACTUATOR_COUNT), jntPositions(ACTUATOR_COUNT),
            jntVelocities(ACTUATOR_COUNT), jntTorques(ACTUATOR_COUNT),
    ...
    while (totalElapsedTime < TASK_TIME_LIMIT_MICRO) {
        ...
        returnFlag = idSolver->CartToJnt(jntPositions, ZERO_ARRAY, ZERO_ARRAY, ZERO_WRENCHES,
                                         jntCmdTorques);
        ...
        if (iterationCount == 1) {
            ...
            desiredValuesLog << desiredPos[0] << "," << desiredPos[1] << "," << desiredPos[2] << ","
                             << desiredTwist[0] << "," << desiredTwist[1] << "," << desiredTwist[2]
                             << std::endl;
            if (desiredValuesLog.is_open()) desiredValuesLog.close();
        }
        ...
        for (int i = 0; i < 6; i++) {
            abag_sched(&abagStates[i], &errors[i], &commands[i],
                       &(pAbagConfigs.at(kcc_conf::ALPHA)[i]),
                       &(pAbagConfigs.at(kcc_conf::BIAS_THRES)[i]),
                       &(pAbagConfigs.at(kcc_conf::BIAS_STEP)[i]),
                       &(pAbagConfigs.at(kcc_conf::GAIN_THRES)[i]),
                       &(pAbagConfigs.at(kcc_conf::GAIN_STEP)[i]));
        }
        ...
        // std::cout << "ee force: " << endEffForce(0) << ", " << endEffForce(1) << ", " <<
        // endEffForce(2) << std::endl; std::cout << "jnt cmd torques: " <<
        // jntCmdTorques.data.transpose() << std::endl;
        ...
        // control
        kc::writeDataRow(logPosX, abagStates[0], totalElapsedTime.count(), errors[0], commands[0],
                         endEffPose.p(0));
        kc::writeDataRow(logPosY, abagStates[1], totalElapsedTime.count(), errors[1], commands[1],
                         endEffPose.p(1));
        kc::writeDataRow(logPosZ, abagStates[2], totalElapsedTime.count(), errors[2], commands[2],
                         endEffPose.p(2));
        kc::writeDataRow(logVelX, abagStates[3], totalElapsedTime.count(), errors[3], commands[3],
                         endEffTwist.p.v(0));
        kc::writeDataRow(logVelY, abagStates[4], totalElapsedTime.count(), errors[4], commands[4],
                         endEffTwist.p.v(1));
        kc::writeDataRow(logVelZ, abagStates[5], totalElapsedTime.count(), errors[5], commands[5],
                         endEffTwist.p.v(2));
    ...
    std::cout << "Number of loops which took longer than specified duration: " << slowLoopCount
              << std::endl;
...
int main(int argc, char** argv) {  // Get file path
    ...
    // Load configurations
    // @Minh's Note: This needs to exist to query the root setting, otherwise will cause segfault
    libconfig::Config cfg;  // TODO: Check what Minh's note is about
    ...
    try {
        const libconfig::Setting& root = kc::loadConfigFile(cfg, config_file_path.c_str());

        kc::loadAbagConfig(root, CTRL_APPROACH, abagConfigs);
        kc::loadKinovaConfig(root, cartForceLimits, hostName, username, password, port,
                             portRealTime);
    ...
    // Load URDF file
    KDL::Tree kinovaTree;
    KDL::Chain kinovaChain;
    kc::loadUrdfModel(urdf_file_path, kinovaTree, kinovaChain);

    // Establish connection with arm and run impedance control
    try {
        ...
        try {
            ...
            impedance_control_posvel(connection.mBaseClient.get(),
                                     connection.mBaseCyclicClient.get(),
                                     connection.mActuatorConfigClient.get(), kinovaChain,
